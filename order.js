...
function setOrder(keyword, mas) {
    ...
    window.localStorage.getItem(selectedDish.category);
}
function addDishToOrder(category, keyword) {
    
    let selectedDishes = JSON.parse(localStorage.getItem('selectedDishes'))
     || [];
    
    // Проверяем, что это блюдо еще не было добавлено
    if (!selectedDishes.includes(category)) {
        selectedDishes.push(category, keyword); 
...
function dinamicCards(mas) {
    ...
    sortedDishes.forEach(dish => {
        ...
        dishCard.querySelector('button').onclick = () => {
            ...
            addDishToOrder(dish.category, dish.keyword);
        };
    });
}
...
document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("try").innerHTML(window.localStorage("tenders"));
